...
| `structure` | 否 | 结构化关系数据：`deps`（依赖）、`exports`（导出）、`inner`（内部结构） |
| `structure.deps` | 否 | 依赖列表，每项：`m`（模块路径）、`use`（符号列表）、`type`（import/call/inherit） |
| `structure.exports` | 否 | 导出列表，每项：`n`（符号名）、`t`（class/function/interface/type/const）、`path`（文件路径） |
| `structure.inner` | 否 | 内部结构，每项：`n`（容器名）、`t`（class/namespace/module）、`has`（成员列表） |
